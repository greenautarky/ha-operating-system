...
if [ -f "$ADDON_IMAGES_JSON" ] && command -v skopeo >/dev/null 2>&1; then
    ...
    for key in $(jq -r '.addons | keys[]' "$ADDON_IMAGES_JSON"); do
        ...
        # RELEASE. Without it, publishing 1.3.2-dev makes a correct pin of 1.2.0
        # look like drift — and because ga_build.sh runs this with
        # GA_SOURCE_DRIFT_STRICT=1, that fails the build. The check exists to
        # find "a released image no device can reach because the pin is behind";
        # a -dev tag is precisely something we do NOT want devices to reach, so
        # counting it here made the guard fire on the wrong event.
        # A pre-release that should ship gets a release tag. That is the point.
        if [ -z "$newest" ]; then
