...
decompress_image() {
  ...
  # Decompress alongside the image (needs ~6GB free) — /tmp is too small
  TMPDIR_WORK="$(mktemp -d "$(dirname "$IMAGE")/verify-sd.XXXXXX")"
  _info "Decompressing image (needs ~6GB, writing to $(dirname "$IMAGE"))…"
  if [[ "$IMAGE" == *.xz ]]; then
