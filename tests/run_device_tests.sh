...
usage() {
    ...
    echo "  crash_detection telemetry environment network ping boot_timing disk_guard watchdog config_verify stress idle_perf onboarding tailscale hardware"
    ...
    if echo "$output" | grep -q "NET_OK"; then
        ...
        if [[ -n "$host_ip" ]]; then
            ...
            if echo "$output" | grep -q "DL_OK"; then
                ...
                for suite in crash_detection telemetry environment network ping boot_timing disk_guard watchdog config_verify stress idle_perf onboarding tailscale hardware; do
