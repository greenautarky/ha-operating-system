...
SUITES_EMU="environment crash_detection boot_timing disk_guard supervisor_health share_publish rauc_slots stage_components publish_services lte_standby usb_net_posture"
# usb_net_posture is NOT here: it reads the source tree (cmdline.txt, the udev
# rule, NetworkManager.conf), which run_device_tests.sh does not ship — on a
# device it produced six failures and four vacuous red-proofs. It stays in
# SUITES_EMU/SUITES_ALL, which run from a checkout.
